...
client.on('group.add', async ctx => {
   const sock = client.sock
   const text = `*Selamat datang +tag member baru di grup +grup semoga betah ya*

❗Jangan lupa gunakan jasa rekber admin agar terhindar penipuan sekecil apa pun transaksi anda terimakasih🙏

ADMIN ON MC REKBER CULIK KALAU GAK ON MISKOL PASTI ON


*MELAYANI*
ADMIN OPEN MC/REBER
STOK AKUN JIKA ADA
JASA EDIT LOGO/SPEK AKUN ML
JASA SUNTIK ALL
SOSMED
JASA NEED AKUN 
JASA POS FREE`
...
client.on('ready', async () => {
  await require('./lib/cekupdate')(client.sock, Func)
})
